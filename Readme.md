...
```mermaid
flowchart TD
    A[Inbound Invoice Payload] --> B[invoice_extractor_node]
    B --> C[contract_grounder_node]
    C --> D[policy_governor_node]
    
    D -->|Variance = $0.00| E[AUTO_APPROVE: Direct ERP Disbursement]
    D -->|Variance >= $500.00| F[STAGE_DISPUTE_HITL: Dispute Package to Human]
    D -->|Unmatched PO| G[REJECT_UNAUTHORIZED: Disbursement Blocked]
```
...
| :--- | :--- | :--- | :--- |
| `invoice_extractor_node` | `invoice-extractor-agent` | `gpt-5-mini-1` | Ingests invoice files/JSON payloads, normalizes currency/totals, and structures line items. |
...
Telemetry captured natively via Azure Application Insights inside the Azure AI Foundry portal:
